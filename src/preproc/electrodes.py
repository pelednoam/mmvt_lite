...
def electrodes_csv_to_npy(subject, ras_file, output_file='', bipolar=False, delimiter=',', electrodes_type=None,
    ...
    electrodes_names_fname = utils.add_str_to_file_name(output_file, '_names', 'csv')
    np.savetxt(electrodes_names_fname, names, fmt='%s', delimiter=delimiter)
    return pos, names


def save_electrodes_names(subject, bipolar=False, delimiter=',', snap=False):
    input_fname = op.join(MMVT_DIR, subject, 'electrodes','electrodes{}_{}positions.npz'.format(
        '_bipolar' if bipolar else '', 'snap_' if snap else ''))
    if not op.isfile(input_fname):
        print('No electrodes position! {}'.format(input_fname))
        return False
    names = np.load(input_fname)['names']
    electrodes_names_fname = op.join(MMVT_DIR, subject, 'electrodes', 'electrodes{}_{}names.csv'.format(
        '_bipolar' if bipolar else '', 'snap_' if snap else ''))
    electrodes_names_fname = utils.add_str_to_file_name(input_fname, '_names', 'csv')
    np.savetxt(electrodes_names_fname, names, fmt='%s', delimiter=delimiter)
    return op.isfile(electrodes_names_fname)
...
def calc_electrodes_types(labels, pos, electrodes_type=None):
    ...
    for index in range(len(labels) - 1):
        ...
        if elc_group1 == elc_group2:
            dist = np.linalg.norm(pos[index + 1] - pos[index])
            dists[elc_group1].append(dist)
            # if elc_group1 == 'LAMY':
            #     print('{} - {}: {} ({}, {})'.format(labels[index], labels[index + 1], dist, pos[index], pos[index + 1]))
...
def find_electrodes_hemis(subject, bipolar, sigma=0, manual=False, electrodes_type=None):
    ...
    if in_dural is None:
        hemis = {elc_name: lateralization_based_on_coordinates(elc_coords)
                 for elc_name, elc_coords in zip(electrodes,electrodes_t1_tkreg)}
    else:
...
def lateralization_based_on_coordinates(coord):
    return 'lh' if coord[0] < 0 else 'rh'
...
def create_raw_data_from_edf(subject, args, downsample_r=1, stat=STAT_DIFF, electrodes_type=None, do_plot=False, overwrite=False):
    ...
    edf_raw = mne.io.read_raw_edf(edf_fname, preload=args.preload, exclude=args.bad_channels)
    print('{}: {:.2f}s'.format(utils.namebase(edf_fname), edf_raw.times[-1]))
    ...
    for cond in args.conditions:
        # todo: Get the from and to sa variable
        seizures = [(annot['description'], annot['onset']) for annot in edf_raw.annotations
                    if annot['description'] == 'Seizure']
        if 'from_t' in cond and 'to_t' in cond and cond['from_t'] == cond['to_t']:
                continue

        if len(seizures) > 0:
            cond['from_t'] = seizures[0][1] + cond['from_t']
            cond['to_t'] = seizures[0][1] + cond['to_t']
        if 'from_t' in cond and 'to_t' in cond and cond['to_t'] != -1:
            if cond['from_t'] == cond['to_t']:
                continue
        ...
        if downsample_r > 1:
            cond_data = utils.downsample_2d(cond_data, downsample_r)
            times = utils.downsample(times, downsample_r)
        ...
        if cond['name'] == 'baseline':
            baseline_fname = op.join(fol, 'electrodes{}_baseline_{}.npy'.format(
                '_bipolar' if args.bipolar else '', utils.namebase(edf_fname)))
            np.save(baseline_fname, cond_data)
            print('Baseline shape: {}'.format(cond_data.shape))
            baseline_data = cond_data# np.mean(cond_data, 1)
        else:
            # import rdp
    ...
    if 'baseline' in conditions:
        ...
        if args.remove_baseline:
            for c in range(data.shape[2]):
                ...
                data[:, :, c] -= baseline_mean #np.tile(baseline_mean, (T, 1)).T
                if args.calc_zscore:
                    data[:, :, c] /= baseline_std #np.tile(baseline.std(axis=1), (T, 1)).T
    ...
    if args.moving_average_win_size > 0:
        ...
        meta_fname = op.join(fol, 'electrodes_meta_data{}_{}.npz'.format(
            '_{}'.format(STAT_NAME[stat]) if len(conditions) > 1 else '', utils.namebase(edf_fname)))
        data_fname = op.join(fol, 'electrodes_data{}_{}.npy'.format(
            '_{}'.format(STAT_NAME[stat]) if len(conditions) > 1 else '', utils.namebase(edf_fname)))
        np.savez(meta_fname, names=labels, conditions=conditions, times=times, sfreq=new_hz)
...
def calc_seizure_times(start_time, seizure_onset, seizure_end, baseline_onset, baseline_end,
    ...
    seizure_end_t = (datetime.strptime(seizure_end, time_format) - start_time).seconds if seizure_end != '-1' else -1
    baseline_start_t = (datetime.strptime(baseline_onset, time_format) - start_time).seconds
    baseline_end_t = (datetime.strptime(baseline_end, time_format) - start_time).seconds if baseline_end != '-1' else -1
    ...
    with open(coloring_fname, 'w') as colors_rgbs_file, open(colors_names_fname, 'w') as colors_names_file:
        ...
        for elec_probs in elecs_probs:
            ...
            if not good_channels is None and elec_name not in good_channels:
                print('Not coloring {}'.format(elec_name))
                continue
...
def get_most_probable_rois(elecs_probs, p_threshold=0.05, good_channels=None, return_hemi_indifferent_rois=True):
    return_one_roi = False
    if not isinstance(elecs_probs, list):
        return_one_roi = True
        elecs_probs = [elecs_probs]
    ...
    if return_hemi_indifferent_rois:
        probable_rois = utils.get_hemi_indifferent_rois(probable_rois)
    return list(probable_rois)[0] if return_one_roi else probable_rois
...
def merge_morph_electrodes(subject_morph_to='fsaverage', bipolar=False):
    fol = op.join(MMVT_DIR, subject_morph_to, 'electrodes')
    output_fname = op.join(fol, 'morphed_electrodes_positions{}.npz'.format('_bipolar' if bipolar else ''))
    output_csv_fname = op.join(fol, 'morphed_electrodes_positions{}.csv'.format('_bipolar' if bipolar else ''))
    files = sorted(glob.glob(op.join(fol, 'electrodes_positions_from_*.npz')), key=utils.natural_keys)
    print('Reading morphed electrodes from {}:'.format(op.join(fol, 'electrodes_positions_from_*.npz')))
    subjects = []
    for fname in files:
        subject = utils.namebase(fname).split('_')[-1]
    ...
    names, pos = [], []
    for fname in files:
        output_subject_csv_fname = op.join(fol, '{}{}.csv'.format(
            utils.namebase(fname), '_bipolar' if bipolar else ''))
        elecs_dict = np.load(fname) # np.savez(output_fname, pos=pos, names=names, pos_org=[])
        inds = [i[1] for i in sorted(utils.enumerate_val_first(elecs_dict['names']), key=utils.natural_keys)]
        # inds = np.argsort(elecs_dict['names']) # key=mu.natural_keys
        elcs_names = elecs_dict['names'][inds]
        elcs_pos = elecs_dict['pos'][inds]
        if bipolar:
            elcs_names, elcs_pos = mono_to_bipoloar(elcs_names, elcs_pos, names_include_subject=True)
        write_electrodes_to_csv(elcs_names, elcs_pos, output_subject_csv_fname)
        names.extend(elcs_names)
        pos.extend(elcs_pos)
    print('Saving morphed electrodes -> {}'.format(output_fname))
    np.savez(output_fname, pos=pos, names=names, pos_org=[], subjects=subjects)
    write_electrodes_to_csv(names, pos, output_csv_fname)
    return op.isfile(output_fname) and op.isfile(output_csv_fname)


def write_electrodes_to_csv(names, pos, output_csv_fname):
    with open(output_csv_fname, 'w', newline='') as csv_file:
        csv_writer = csv.writer(csv_file, delimiter=',')
        csv_writer.writerow(['name', 'x', 'y', 'z'])
        for elc_name, elc_pos in zip(names, pos):
            csv_writer.writerow([elc_name, *elc_pos])


def mono_to_bipoloar(names, pos, names_include_subject=False):
    bipolar_names, bipolar_pos = [], []
    for index in range(len(names) - 1):
        elc1_name = names[index]
        elc2_name = names[index + 1]
        elc_group1, elc_num1 = utils.elec_group_number(
            elc1_name.split('_')[-1] if names_include_subject else elc1_name)
        elc_group2, elc_num2 = utils.elec_group_number(
            elc2_name.split('_')[-1] if names_include_subject else elc2_name)
        if elc_group1 == elc_group2:
            elec_name = '{0}-{2}-{1}'.format(*elc1_name.split('_'), elc2_name.split('_')[1]) if names_include_subject else\
                '{}-{}'.format(elc2_name, elc1_name)
            bipolar_names.append(elec_name)
            bipolar_pos.append(pos[index] + (pos[index + 1] - pos[index]) / 2)
    return bipolar_names, bipolar_pos


def create_morphed_electrodes_coloring(subject='fsaverage', coloring_fname=''):
    coloring_fname = 'morphed_electrodes_coloring.csv' if coloring_fname == '' else coloring_fname
    fol = op.join(MMVT_DIR, subject, 'electrodes')
    input_fname = op.join(fol, 'morphed_electrodes_positions.npz') #electrodes_morphed_from_subjects.npz')
    electrodes_dict = np.load(input_fname)
    colors_rgb_and_names = cu.get_distinct_colors_and_names(len(electrodes_dict['subjects']), boynton=True)
    group_colors = {subject: color for subject, color in zip(electrodes_dict['subjects'], colors_rgb_and_names)}
    coloring_fname = op.join(MMVT_DIR, subject, 'coloring', coloring_fname)
    print('Writing {}'.format(coloring_fname))
    with open(coloring_fname, 'w') as colors_file:
        for elc_name in electrodes_dict['names']:
            elc_subject = elc_name.split('_')[0]
            colors_file.write('{},{},{},{}\n'.format(elc_name, *group_colors[elc_subject][0]))
    return op.isfile(coloring_fname)
...
def run_ela(subject, atlas, bipolar, overwrite=False, elc_r=3, elc_len=4, electrodes_type=None, n_jobs=-1,
            save_mmvt_output=True, return_output_fname=False):
    ...
    if save_mmvt_output:
        mmvt_electrodes_root = utils.make_dir(op.join(MMVT_DIR, subject, 'electrodes'))
        mmvt_ela_fname = op.join(mmvt_electrodes_root, output_name)
    else:
        mmvt_electrodes_root, mmvt_ela_fname = '', ''
    ...
    if electrodes_type is not None:
        cmd_args.extend(['--electrodes_type', str(electrodes_type)])
    args = find_rois_main.get_args(cmd_args)
    find_rois_main.run_for_all_subjects(args)
    if not op.isfile(output_fname):
        return False, output_fname if return_output_fname else False
    else:
        if save_mmvt_output:
            utils.copy_file(output_fname, mmvt_ela_fname)
        return True, output_fname if return_output_fname else True
...
def create_volume_mask(subject, bipolar=False, overwrite=True):
    from src.preproc import anatomy
    mask_output_fname = op.join(MMVT_DIR, subject, 'freeview', 'electrodes_vol_mask.npy')
    if op.isfile(mask_output_fname) and not overwrite:
        print('The file already exists! Use --overwrite_volume_mask 1 to overwrite')
        return True
    input_file = op.join(
        MMVT_DIR, subject, 'electrodes', 'electrodes{}_positions.npz'.format('_bipolar' if bipolar else ''))
    if not op.isfile(input_file):
        print('You first need to create the electrodes location file:\n')
        print('python3 -m src.prepro.electrodes -s {} -f convert_electrodes_pos'.format(subject))
        return False
    electrodes_pos = np.load(input_file)['pos']
    t1_data, t1_header = anatomy.get_data_and_header(subject, 'T1.mgz')
    ...
    for vox in surface_voxels:
        for (i, j, k) in product([-1, 0, 1], [-1, 0, 1], [-1, 0, 1]):
            surface_vol[tuple(vox + [i, j, k])] = 1
    # surface_vol[(surface_vol[:, 0]), (surface_vol[:, 1]), (surface_vol[:, 2])] = 1
    # surface_vol[[surface_voxels[:, k] for k in range(2)]] = 1
    print('{:.6f}% voxels are electrodes'.format(
        len(np.where(surface_vol)[0])/(t1_data.shape[0] * t1_data.shape[1])))
    np.save(mask_output_fname, surface_vol)
    return op.isfile(mask_output_fname)


def calc_dist_to_surface(subject, atlas, bipolar=False, n_jobs=4):
    electrodes_labels_min_dists_fname = op.join(MMVT_DIR, subject, 'electrodes', 'electrodes_labels_min_dists.npz')
    csv_output_fname = op.join(MMVT_DIR, subject, 'electrodes', 'electrodes_labels.csv')

    # Read electrodes and pial info
    electrdes_names, electrdes_pos = read_electrodes_file(subject, bipolar=bipolar)
    pial_verts = {hemi: utils.read_pial(subject, MMVT_DIR, hemi, return_only_verts=True)
                  for hemi in utils.HEMIS}
    thickness = None
    if utils.both_hemi_files_exist(op.join(SUBJECTS_DIR, subject, 'surf', '{hemi}.thickness')):
        thickness = {hemi: gu.read_thickness(op.join(SUBJECTS_DIR, subject, 'surf', '{}.thickness'.format(hemi)))
                     for hemi in utils.HEMIS}

    # Read labels, and remove unknown labels
    ...
    if len(labels_names) == 0:
        print('calc_dist_to_surface: no labels for {}!'.format(atlas))
        return False

    electrodes_labels_dists = np.empty((len(electrdes_pos), len(labels)))
    electrodes_labels_dists.fill(np.nan)
    electrodes_labels_thickness = np.empty((len(electrdes_pos), len(labels)))
    electrodes_labels_thickness.fill(np.nan)
    for label_ind, label in enumerate(labels):
        label_pos = pial_verts[label.hemi][label.vertices]
        electrodes_label_dists = cdist(electrdes_pos, label_pos)
        electrodes_labels_dists[:, label_ind] = electrodes_label_dists.min(1)
        if thickness is not None:
            electrodes_labels_vertices_ind = electrodes_label_dists.argmin(1)
            electrodes_labels_thickness[:, label_ind] = [
                thickness[label.hemi][vert_ind] for vert_ind in electrodes_labels_vertices_ind]

    print('Writing npz results in {}'.format(electrodes_labels_min_dists_fname))
    np.savez(electrodes_labels_min_dists_fname, electrodes_labels_dists=electrodes_labels_dists,
             electrdes_names=electrdes_names, labels_names=labels_names,
             electrodes_labels_thickness=electrodes_labels_thickness)

    print('Writing results in {}'.format(csv_output_fname))
    electrodes_closest_labels_inds = electrodes_labels_dists.argmin(1)
    electrodes_closest_labels_dists = electrodes_labels_dists.min(1)
    with open(csv_output_fname, 'w', newline='', encoding='utf-8') as csv_file:
        wr = csv.writer(csv_file, quoting=csv.QUOTE_NONE)
        wr.writerow(['Electrode', 'Closest label', 'Distance',
                     'Cortical thickness' if thickness is not None else '',
                     'Within Cortex' if thickness is not None else ''])
        for elc_ind, (label_ind, label_dist, elc_name) in \
                enumerate(zip(electrodes_closest_labels_inds, electrodes_closest_labels_dists, electrdes_names)):
            thick = electrodes_labels_thickness[elc_ind, label_ind] if thickness is not None else 0
            within = 'True' if label_dist < thick else 'False'
            wr.writerow([elc_name, labels_names[label_ind], '{:.5f}'.format(label_dist),
                         '{:.5f}'.format(thick) if thickness is not None else '',
                         within if thickness is not None else ''])
    return op.isfile(electrodes_labels_min_dists_fname)
...
def main(subject, remote_subject_dir, args, flags):
    ...
    if utils.should_run(args, 'save_electrodes_names'):
        flags['save_electrodes_names'] = save_electrodes_names(
            subject, bipolar=args.bipolar, delimiter=args.delimiter, snap=args.snap)
    ...
    if 'morph_electrodes' in args.function:
        flags['morph_electrodes'] = morph_electrodes(
            ...
            args.overwrite_morphed_elecctrodes, args.n_jobs)

    if 'merge_morph_electrodes' in args.function:
        flags['merge_morph_electrodes'] = merge_morph_electrodes(subject, args.bipolar)

    if 'create_morphed_electrodes_coloring' in args.function:
        flags['create_morphed_electrodes_coloring'] = create_morphed_electrodes_coloring(subject)
    ...
    if 'create_raw_data_from_edf' in args.function:# and not args.task is None:
        flags['create_raw_data_from_edf'] = create_raw_data_from_edf(
            subject, args, args.downsample_r, overwrite=args.overwrite_raw_data, electrodes_type=args.electrodes_type)
    ...
    if utils.should_run(args, 'calc_dist_to_surface'):
        flags['calc_dist_to_surface'] = calc_dist_to_surface(subject, args.atlas, args.bipolar, args.n_jobs)
...
def read_cmd_args(argv=None):
    ...
    parser.add_argument('--overwrite_volume_mask', required=False, default=0, type=au.is_true)
