...
def namebase(fname):
    sep = '/' if fname.startswith('/') else op.sep
    name_with_ext = fname.split(sep)[-1]
...
def change_fname_extension(fname, new_extension, new_suffix=''):
    return op.join(get_fname_folder(fname), '{}{}.{}'.format(
        namebase(fname), new_suffix, new_extension))
...
def insert_keyframe_to_custom_prop(obj, prop_name, value, keyframe):
    ...
    if len(prop_name) > 63:
        print('keyframe\'s key can be up to 63 characters! {} has {}!'.format(prop_name, len(prop_name)))
        prop_name = prop_name[:63]
    obj[prop_name] = value
...
def create_and_set_material(obj, default_color=(1, 1, 1, 1)):
...
def import_slice_materials(blend_fname):
    slices_names = ['coronal', 'sagital', 'axial']
    try:
        for slice_name in slices_names:
            cur_mat_name = '{}_plane_mat'.format(slice_name)
            if bpy.data.materials.get(cur_mat_name) is None:
                bpy.ops.wm.append(
                    link=False, directory=blend_fname + '\\Material\\', filename=cur_mat_name)
        return all([bpy.data.materials.get('{}_plane_mat'.format(slice_str)) is not None
                    for slice_str in slices_names])
    except:
        return False
...
def create_bezier_curve(pos1, pos2, layers_array, bevel_depth=0.1, resolution_u=1, conn_name=''):
    ...
    curve.splines[0].bezier_points[0].co = pos1
    curve.splines[0].bezier_points[1].co = pos2
...
#TODO rewrite this function oct 2020
def delete_hierarchy(parent_obj_name, exceptions=(), delete_only_animation=False, delete_parent_obj=False):
...
def fix_atlas_name(subject, atlas, subjects_dir=''):
    if atlas in ['dkt', 'dkt40', 'aparc.DKTatlas', 'aparc.DKTatlas40']:
        if os.environ.get('FREESURFER_HOME', '') != '':
            ...
            dkt_exist = atlas_exist(subject, 'aparc.DKTatlas', subjects_dir)
            dkt40_exist = atlas_exist(subject, 'aparc.DKTatlas40', subjects_dir)
            if dkt_exist:
                atlas = 'aparc.DKTatlas'
            elif dkt40_exist:
                atlas = 'aparc.DKTatlas40'
...
def time_to_go(now, run, runs_num, runs_num_to_print=10, thread=-1, do_write_to_stderr=False, to_hours=False):
    if run % runs_num_to_print == 0 and run != 0:
        ...
        if to_hours:
            ...
            time_char = 's'
        if thread > 0:
            str = '***** {}: {}/{}, {:.2f}{}, {:.2f}{} to go!'.format(
                thread, run, runs_num, time_took, time_char, more_time, time_char)
        else:
            str = '***** {}/{}, {:.2f}{}, {:.2f}{} to go!'.format(
                run, runs_num, time_took, time_char, more_time, time_char)
...
def csv_file_reader(
        csv_fname, delimiter=',', skip_header=0, encoding=None, find_encoding=False, max_lines=0, return_generator=True):
    import csv
    ...
    with open(csv_fname, 'r', encoding=encoding) as csvfile:
        ...
        for line_num, line in enumerate(reader):
            ...
            if return_generator:
                ...
                lines.append([val.strip() for val in line])
...
def redirect_output_to_file(fname, overwrite=True):
...
def both_hemi_files_exist(file_template):
    ...
    if '*' not in file_template:
        if '{hemi}' not in file_template:
            file_template = file_template.replace('-rh.stc', '{hemi}.stc').replace('-lh.stc', '{hemi}.stc')
        if '{hemi}' not in file_template:
            file_template = '{}-{}.stc'.format(file_template, '{hemi}')
        return op.isfile(file_template.format(hemi='rh')) and op.isfile(file_template.format(hemi='lh'))
    else:
        if '{hemi}' in file_template:
...
def merge_2_maps(x1, x2, thresh):
    x, x_join = {}, {}
    for hemi in HEMIS:
        join_x1_indices = np.where((x1[hemi] > thresh) & (x2[hemi] > thresh) & (x1[hemi] > x2[hemi]))[0]
        join_x2_indices = np.where((x1[hemi] > thresh) & (x2[hemi] > thresh) & (x2[hemi] > x1[hemi]))[0]
        x_join[hemi] = np.zeros(x1[hemi].shape)
        x_join[hemi][join_x1_indices] = x1[hemi][join_x1_indices]
        x_join[hemi][join_x2_indices] = x2[hemi][join_x2_indices]
        bigger_x1_indices = np.where((x1[hemi] > x2[hemi]) & (x1[hemi] > thresh))[0]
        bigger_x2_indices = np.where((x2[hemi] > x1[hemi]) & (x2[hemi] > thresh))[0]
        x[hemi] = np.zeros(x1[hemi].shape)
        x[hemi][bigger_x1_indices] = x1[hemi][bigger_x1_indices]
        x[hemi][bigger_x2_indices] = -x2[hemi][bigger_x2_indices]
    return x, x_join
